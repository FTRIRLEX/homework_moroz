...
export const revrot = (str: string, sz: number) => {
  ...
  const checkEven = (s: string) => (Array as any).prototype.reduce.call(s, (acc: number, val: number) => acc + Number(val) ** 3, 0) % 2 === 0;
  const reverse = (s: string) => s.split('').reverse().join('');
  const rotate = (s: string) => s.slice(1) + s.slice(0, 1);

  const arr = [];
  for (let i = 0; i < ln; i += sz) {
    arr.push(i+sz <= ln ? str.slice(i, i+sz) : '');
  }
  return arr.map(str => checkEven(str) ? reverse(str) : rotate(str)).join('');
};
